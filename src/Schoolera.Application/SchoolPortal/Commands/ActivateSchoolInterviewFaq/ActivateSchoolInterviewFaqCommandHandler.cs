...
namespace Schoolera.Application.SchoolPortal.Commands.ActivateSchoolInterviewFaq;

public sealed record ActivateSchoolInterviewFaqCommand(Guid SchoolId, Guid ItemId)
    : IRequest<Result<SchoolInterviewFaqDetailDto>>;

public sealed class ActivateSchoolInterviewFaqCommandHandler(
    ISchoolPortalAccess portalAccess,
    ICmsRepository cmsRepository,
    ISchoolPortalAuditWriter auditWriter,
    IUnitOfWork unitOfWork,
    IStringLocalizer<SchoolPortalMessages> localizer,
    ILogger<ActivateSchoolInterviewFaqCommandHandler> logger)
    : IRequestHandler<ActivateSchoolInterviewFaqCommand, Result<SchoolInterviewFaqDetailDto>>
{
    public async Task<Result<SchoolInterviewFaqDetailDto>> Handle(
        ActivateSchoolInterviewFaqCommand request,
        CancellationToken cancellationToken)
    {
        ...
        await auditWriter.WriteAsync(
            access!.UserId,
            SchoolPortalAuditActions.InterviewFaqActivated,
            "FaqItem",
            item.Id.ToString(),
            "Activated school interview FAQ.",
            cancellationToken);

        logger.LogInformation("Activated school interview FAQ {ItemId}.", item.Id);
