...
public sealed record DeactivateSchoolAdmissionRequirementCommand(
    Guid SchoolId,
    Guid RequirementId) : IRequest<Result<SchoolAdmissionRequirementDetailDto>>;

public sealed class DeactivateSchoolAdmissionRequirementCommandHandler(
    ISchoolPortalAccess portalAccess,
    ISchoolAdmissionRequirementRepository requirementRepository,
    IUnitOfWork unitOfWork,
    IStringLocalizer<SchoolPortalMessages> localizer,
    ILogger<DeactivateSchoolAdmissionRequirementCommandHandler> logger)
    : IRequestHandler<DeactivateSchoolAdmissionRequirementCommand, Result<SchoolAdmissionRequirementDetailDto>>
{
    public async Task<Result<SchoolAdmissionRequirementDetailDto>> Handle(
        DeactivateSchoolAdmissionRequirementCommand request,
        CancellationToken cancellationToken)
    {
        ...
        await requirementRepository.AddAuditAsync(
            new SchoolAdmissionRequirementAudit(
                request.SchoolId,
                entity.Id,
                SchoolAdmissionRequirementAuditActions.Deactivated,
                access.Data.UserId,
        ...
        logger.LogInformation(
            "Deactivated admission requirement {RequirementId} for school {SchoolId}.",
            entity.Id,
            request.SchoolId);
