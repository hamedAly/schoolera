...
public sealed record OpenInterviewAssessmentSlotCommand(
    Guid SchoolId, Guid SlotId, byte[]? RowVersion) : IRequest<Result<InterviewAssessmentSlotDto>>;

public sealed class OpenInterviewAssessmentSlotCommandHandler(
    ...
    ILogger<OpenInterviewAssessmentSlotCommandHandler> logger)
    : IRequestHandler<OpenInterviewAssessmentSlotCommand, Result<InterviewAssessmentSlotDto>>
{
    public async Task<Result<InterviewAssessmentSlotDto>> Handle(
        OpenInterviewAssessmentSlotCommand request, CancellationToken cancellationToken)
    {
        _ = unitOfWork;
        ...
        var auditAction = slot.Status == Domain.Enums.SlotStatus.Closed ? "Reopened" : "Opened";
        ...
        if (openResult != AtomicSlotOpenResult.Succeeded)
            return InterviewAssessmentSlotSupport.Fail<InterviewAssessmentSlotDto>(
                localizer, "ConcurrentUpdate", SchoolPortalErrorCodes.ConcurrentUpdate);
        logger.LogInformation("Opened interview assessment slot {SlotId}.", slot.Id);
