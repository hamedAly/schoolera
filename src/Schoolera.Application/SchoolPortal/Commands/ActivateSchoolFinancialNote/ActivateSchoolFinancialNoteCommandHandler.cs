...
public sealed record ActivateSchoolFinancialNoteCommand(
    Guid SchoolId,
    Guid ItemId) : IRequest<Result<SchoolFinancialNoteDto>>;

public sealed class ActivateSchoolFinancialNoteCommandHandler(
    ISchoolPortalAccess portalAccess,
    ISchoolPortalRepository repository,
    IUnitOfWork unitOfWork,
    IStringLocalizer<SchoolPortalMessages> localizer,
    ILogger<ActivateSchoolFinancialNoteCommandHandler> logger)
    : IRequestHandler<ActivateSchoolFinancialNoteCommand, Result<SchoolFinancialNoteDto>>
{
    public async Task<Result<SchoolFinancialNoteDto>> Handle(
        ActivateSchoolFinancialNoteCommand request,
