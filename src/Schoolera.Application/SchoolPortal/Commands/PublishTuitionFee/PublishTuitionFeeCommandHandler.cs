...
public sealed record PublishTuitionFeeCommand(
    Guid SchoolId,
    Guid FeeId) : IRequest<Result<TuitionFeeDto>>;

public sealed class PublishTuitionFeeCommandHandler(
    ISchoolPortalAccess portalAccess,
    ISchoolPortalRepository repository,
    IUnitOfWork unitOfWork,
    IStringLocalizer<SchoolPortalMessages> localizer,
    ILogger<PublishTuitionFeeCommandHandler> logger)
    : IRequestHandler<PublishTuitionFeeCommand, Result<TuitionFeeDto>>
{
    public async Task<Result<TuitionFeeDto>> Handle(
        PublishTuitionFeeCommand request,
        CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation(
            "Published tuition fee {FeeId} for school {SchoolId}.",
            fee.Id,
            request.SchoolId);
