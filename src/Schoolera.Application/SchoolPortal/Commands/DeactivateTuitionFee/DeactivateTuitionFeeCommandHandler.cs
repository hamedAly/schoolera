...
public sealed record DeactivateTuitionFeeCommand(
    Guid SchoolId,
    Guid FeeId) : IRequest<Result<TuitionFeeDto>>;

public sealed class DeactivateTuitionFeeCommandHandler(
    ISchoolPortalAccess portalAccess,
    ISchoolPortalRepository repository,
    IUnitOfWork unitOfWork,
    IStringLocalizer<SchoolPortalMessages> localizer,
    ILogger<DeactivateTuitionFeeCommandHandler> logger)
    : IRequestHandler<DeactivateTuitionFeeCommand, Result<TuitionFeeDto>>
{
    public async Task<Result<TuitionFeeDto>> Handle(
        DeactivateTuitionFeeCommand request,
