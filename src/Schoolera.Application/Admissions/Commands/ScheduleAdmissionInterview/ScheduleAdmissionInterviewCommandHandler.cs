...
public sealed record ScheduleAdmissionInterviewCommand(
    Guid SchoolId,
    Guid ApplicationId,
...
public sealed class ScheduleAdmissionInterviewCommandHandler(
    ...
    ILogger<ScheduleAdmissionInterviewCommandHandler> logger)
    : IRequestHandler<ScheduleAdmissionInterviewCommand, Result<SchoolAdmissionApplicationDetailDto>>
{
    public async Task<Result<SchoolAdmissionApplicationDetailDto>> Handle(
        ScheduleAdmissionInterviewCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (request.Body.SlotId is { } slotId)
        {
            ...
            if (selectedSlot is null || selectedSlot.Status != SlotStatus.Open ||
                selectedSlot.Kind != SlotKind.Interview ||
        ...
        if (selectedSlot is not null)
        {
            var idempotency = await slotRepository.CheckSchoolProposalIdempotencyAsync(
                application.Id, SlotKind.Interview, requestedProposalAction, selectedSlot.Id,
        ...
        {
            return AdmissionResults.Failure<SchoolAdmissionApplicationDetailDto>(
                "Online instructions are required for online interviews.",
                AdmissionErrorCodes.AppointmentInvalid);
        }
        ...
        AdmissionInterviewAppointment? existingAppointment = null;
        var isNoShowReplacement = false;

        if (request.IsReschedule)
        {
            if (!AdmissionTransitionPolicy.CanSchoolRescheduleInterview(application.Status))
            {
                return AdmissionResults.Failure<SchoolAdmissionApplicationDetailDto>(
            ...
            existingAppointment = application.InterviewAppointments?
            ...
            {
                var noShow = application.InterviewAppointments?
                    .OrderByDescending(item => item.CreatedAtUtc)
                ...
                if (!isNoShowReplacement)
                    return AdmissionResults.Failure<SchoolAdmissionApplicationDetailDto>(
                        "No reschedulable interview found.",
                        AdmissionErrorCodes.AppointmentNotFound);
            }

            historyAction = AdmissionHistoryActions.InterviewRescheduled;
        }
        else
        {
            ...
            var active = application.InterviewAppointments?.FirstOrDefault(item => item.IsActiveReservation);
            ...
            historyAction = AdmissionHistoryActions.InterviewScheduled;
        }

        async Task MutateAsync(CancellationToken token)
        {
            AdmissionInterviewAppointment changedAppointment;
            ...
            {
                if (application.Status != AdmissionApplicationStatus.InterviewRequired)
            ...
            var appointmentAction = new AdmissionAppointmentActionHistory(
                application.Id, changedAppointment.Id, SlotKind.Interview,
            ...
            await AdmissionParentNotificationSupport.EnqueueAsync(
                ...
                application,
                request.IsReschedule ? NotificationEventType.InterviewRescheduled
                    : NotificationEventType.InterviewScheduled,
                $"{(request.IsReschedule ? "interview-rescheduled" : "interview-scheduled")}:{appointmentAction.Id:N}",
                token);
        }
        ...
        if (selectedSlot is not null)
        {
            var assignment = await slotRepository.ExecuteAtomicSchoolProposalAsync(
                ...
                existingAppointment?.Id, application.Id, SlotKind.Interview, requestedProposalAction,
        ...
        logger.LogInformation(
            "{Action} interview for application {ApplicationId}.",
            historyAction,
            loaded.Id);
